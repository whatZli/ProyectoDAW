...
// Si es un 1 es obligatorio, si es un 0 no lo es
    ...
    public static function comprobarEntero($integer, $max = PHP_INT_MAX, $min = -PHP_INT_MAX, $obligatorio = 0) {  //AÑADIDOS VALORES POR DEFECTO Y AHORA DETECTA EL 0
        ...
        if ($integer != '0' && !empty($integer) && !filter_var($integer, FILTER_VALIDATE_INT)) {  //HE VARIADO ESTO (COMPRUEBA SI ES 0, EVITA MOSTRAR MENSAJE SI ES OPCIONAL Y COMPRUEBA QUE SEA UN ENTERO)
            ...
            if (is_null($mensajeError)) { //Añadido para que sólo compruebe mayor o menor si no hay errores
                if ($integer > $max) {
...
// Si el parámetro $obligatorio es un 1 es obligatorio, si es un 0 es opcional
     ...
     * @return null|string Devuelve null si es correcto o un mensaje de error en caso de que lo haya.
     */
    public static function validarFecha($fecha, $fechaMaxima = "2999-12-12", $fechaMinima = "1900-01-01", $obligatorio = 0) { //REDISEÑO TOTAL Y AÑADIDOS PARAMETROS INICIALES
        $mensajeError = null;
        ...
        if (is_bool($fechaFormateada) && !empty($fecha)) {
            ...
            if (!empty($fecha) && ($fechaFormateada < $fechaMinima) && ($fechaFormateada > $fechaMaxima)) {//Cambiados símbolos de > y < y &&
...
// Devuelve True si es una fecha valida y un false si no la es
     ...
     * @version 1.0	Comprueba que un códgo de color es válido. Ejemplo de código: #39cf27
     * @param $color Color que se va a comprobar. 
     * @param $obligatorio Valor booleano indicado mediante 1, si es obligatorio o 0 si no lo es. 
     * 		En caso de no pasar este parámetro, por defecto se indica que no es obligatorio. 
     * @return null -> Si la variable errores es null o en su defecto contiene tan sólo espacios en blanco
     * $mensajeError -> Que contiene una cadena con los errores que han surgido concatenados.
     */
    public static function validarColor($color, $obligatorio = 0) {
        $mensaje = null;
        $patron = "/^#+[0-9a-f]{6}$/";
        if ($obligatorio == 1) {
            $mensajeError = self::comprobarNoVacio($color);
        }
        if (!preg_match($patron, $color) && !empty($color)) {
            $mensaje .= "El color debe estar formado por # y 6 caracteres hexadecimales.";
        }
        return $mensaje;
    }
